# 07 â€” MCP Gateway, Tool Registration & Validation/Invalidation

> **Status & maturity (read first).** The `[Impl]` / `[Cfg]` tags below describe *design intent
> and the reference implementation* â€” not production authorization. For the authoritative
...
> endpoint through which every tool call flows. Grounded in [`../SOURCES.md`](../SOURCES.md) Â§1.
> Controls are marked **[Impl]** = platform-implemented or **[Cfg]** = customer-configured.

## 1. The gateway â€” one secure tool endpoint
...
[`02-REFERENCE-ARCHITECTURE.md`](02-REFERENCE-ARCHITECTURE.md) Â§3: every model call, tool call, and
retrieval is brokered here. An agent never holds a long-lived credential to a downstream system and
never talks to an MCP server directly â€” it asks the gateway, which decides, mints a scoped token, and
...
## 2. The tool registry â€” nothing is reachable until it is registered

**Every MCP server and every tool is registered in the gateway tool registry â€” with scopes and
validation rules â€” before any agent may reference it.** An agent manifest can only grant tools by
...
[`04-AGENT-ONBOARDING-STANDARD.md`](04-AGENT-ONBOARDING-STANDARD.md) Â§2). The registry is the
...
## 3. Tool validation â€” what must pass before a tool goes `active`
...
3. **Allow-listing.** Only registered servers/tools are reachable. The gateway operates
   **deny-by-default** â€” an unknown tool id, an unknown server, or an undeclared scope is denied. **[Impl]**
...
When the gateway permits a call, it computes `permitted â‡” agent grant âˆ© user entitlement`
(least-privilege as an *intersection* â€” the agent can never exceed the human it acts for) and then
...
fully compromised agent process holds no standing credential to any backend â€” only momentary,
narrowly-scoped tokens the gateway chose to issue. **[Impl]**

## 5. Invalidation & revocation â€” failing closed on a bad tool
...
- **Status flip to `revoked`.** Setting a registry entry to `revoked` (or `deprecated`) takes effect
  at the gateway immediately. Because every call is brokered, no agent can route around it â€” the next
...
- **Audit.** Every revocation, every subsequent denied call, and the reason are written to the
  ...
  [`02-REFERENCE-ARCHITECTURE.md`](02-REFERENCE-ARCHITECTURE.md) Â§6). **[Impl]**
...
[`04-AGENT-ONBOARDING-STANDARD.md`](04-AGENT-ONBOARDING-STANDARD.md) Â§3). New MCP servers/tools are
...
Author tool definition (schema + scopes + data classes + provenance)
  â†’ Register in gateway tool registry (status: candidate)
  â†’ Validate: schema Â· scope declaration Â· allow-list Â· signature/provenance Â· data-class compat
  â†’ Wire backend auth (IAM role / OAuth scope) + secure token vault
  â†’ Promote to status: active
  â†’ Agents may now grant the tool by id in their manifests (CI enforces declared scope)
  â†’ Day-2: deprecate or revoke via status flip; tokens revoked; deny-list updated; audited
```

New **models** onboard analogously: registered, given an inference profile + guardrail policy, and
referenced by profile ARN â€” so adding or swapping a model is a config change, not a re-architecture.
...
- **Architect:** one broker, one registry, one validation pipeline â€” tools are inherited and governed,
...
**Update (Run 10, 2026-07-07):** the portable MCP gateway pattern is now live-validated end-to-end â€”
...
(`requester == sub`) â€” an arbitrary, replayed, expired, or unbound string is denied, and the gate
fails closed if no ledger is wired, so it is no longer presence-only â€” fail-closed masking,
...
full 9-clause ALLOW-iff predicate â€” the same code the offline suite tests) and its boundary masking
...
clean account (2026-07-12)** â€” stack `aegis-mcp-gateway-b3`, account 111122223333, us-east-1, then torn
down with zero residual. Over HTTPS with a Cognito ID token the reviewed engine returned **ALLOW**
(`kb.search_policy`), **ALLOW + masked** (`ticket.create_draft` â€” the reviewed masker redacted SSN/email
in both the response and the append-only audit row), **DENY** (`db.drop` â€” deny-by-default), and
**APPROVAL_REQUIRED** (`ticket.submit` â€” human gate). The deny/gate strings are
