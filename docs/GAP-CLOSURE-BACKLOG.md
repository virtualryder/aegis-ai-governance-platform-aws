...
| PAR-1 | **PV / EDU parity with the 2026-09-05 controls**: capture-all trail + lineage (#168), enforcement perimeter (endpoint policy, bypass alarm, CMK invocation store), output guardrail as IaC + grounded drafter (#166/#190), WAFv2 on Cognito (#170), authoritative Cedar context resolver (#3) | **IN PROGRESS (P1) — steps 1–3 of 5 DONE 2026-09-06** (PV `6381f23`+`288bba1`, EDU `403ca20`+`339fd71`: capture-all lineage, enforcement perimeter, regulated invocation store, L6 restore-aware logging, L9 endpoints, L12 pinned drafter, L3 CMK families, output guardrail as IaC with contextual grounding + grounded drafter carrying the deterministic assessment (the L14 shape; EDU's stale draft schema corrected) — offline-gated: 10 CDK assertions + drafter unit tests + full-switch synth per pack; parity 11/13). Remaining: WAFv2 on Cognito, authoritative Cedar context resolver; manifests still UNSIGNED (#202 parity) | port the benefits stacks (lineage/observability/network/identity/compute) construct by construct with the benefits CDK tests; `docs/PACK-PARITY.md` flips ❌→✅ only from the code | `tools/check_pack_parity.py --check`; each pack's CDK tests; then REL-5 live |
...
Each of L1–L16 is closed with a test or an exact live assertion (L6 closed 2026-09-06; L14–L16 found by the full-portfolio gate on 2026-09-06 and closed on benefits main, attempt 4 re-running from zero). The gate itself (`scripts/tier1_regate.py`) is now the
