...
| L34 | **OPEN** - the CloudTrail arm is still exposed to the L33 defect | With the audit arm fixed, attempt 18's lineage failure moved to the other side: 11 audit lines against 10 invokes, `audited_not_invoked:assess_eligibility`. `read_cloudtrail_capture` builds its query from four `like` predicates, so it is exposed to exactly the same silent-drop defect L33 proved. An attempted fix - re-read the window unfiltered whenever the filtered query looked short - made it **worse**: invokes went 10 -> 21 because the dedupe key could not separate distinct invokes sharing a one-second `eventTime`, and seven tools flipped to `invoked_not_audited`. Reverted. That is **L21e's lesson walked into a second time in the same session**: a change to one arm of a two-sided comparison is not a fix. | OPEN, deliberately, rather than half-fixed. The right shape is a dedupe key that includes the CloudTrail `eventID` (unique per event) instead of a timestamp tuple, developed and tested against RECORDED fixtures from the four runs already captured - not against live AWS, where each iteration costs ~90 minutes and real spend. Until then `LIN_zero_orphans` cannot be trusted to distinguish a real coverage gap from a dropped row, and the gate's own output says so. |
