...
| RT-3 | Propagate RT-1/RT-2 to PV / EDU / Housing runtimes (same `lib/runtime` template) | **DONE for PV + EDU 2026-09-06 (IaC-asserted); Housing OPEN** — Housing's runtime is the pre-1.9.0 template and needs the full benefits runtime port first (REL-5 prerequisite) | PV `2020cdd`, EDU `5a2a33d`: `validate_input`, guardrail on the runtime model, IaC execution role + drafter mandatory-guardrail condition, `_configure.sh` refusal | PV/EDU: 5 runtime tests + 2 CDK tests each (206 / 204 collected); live runtime gate on the IaC role per pack is REL-5 |
...
Each of L1–L13 is closed with a test or an exact live assertion (L6 closed 2026-09-06). The gate itself (`scripts/tier1_regate.py`) is now the
