...
| "Is the *deployed* authorizer the same engine you reviewed, or a subset?" | **The reviewed engine now (B3, 2026-07-12).** The inline governance subset (allow-list dict + one-line regex) is **deleted** from `infra/golden-pilot/mcp-gateway.yaml`; the authorizer Lambda imports the reviewed `platform_core.policy_engine` (full 9-clause predicate) + `platform_core.masker` (fail-closed) from a **Lambda layer** pre-staged from the single source of truth (`prepare_layer.sh`), so the deployed engine cannot drift from the offline-tested one. cfn-lint-clean; ALLOW/DENY/APPROVAL decisions + masking verified locally against the staged layer. Remaining: a live clean-account redeploy of the layer-backed authorizer. |
