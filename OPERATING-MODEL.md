# Operating Model — Aegis (platform)
...
| 2 | **Who approves consequential actions?** | The framework-enforced human-approval gate (bound, single-use, SoD tokens) — the *mechanism* | Names the approver role and staffs it (**the platform owner / governance board**); the human makes every consequential decision | Accel: R (mechanism) · Customer: A (decision) |
...
| 6 | **Who answers audit requests?** | Append-only + WORM audit, control matrices (NIST/OWASP), the assurance packet, this operating model | Produces the evidence to the auditor; owns the cross-domain governance audit response (**Compliance / GRC**) | Accel: C · Customer: R/A |
...
3. Business reviewer (the platform owner / governance board) and the AI/ML owner approve through the customer's change process.
...
- [ ] Approver role staffed (the platform owner / governance board); SoD enforced (`STRICT_APPROVAL=1` / bound approvals only).
...
- [ ] GRC owner for cross-domain governance audit; audit-evidence export tested from the append-only trail.
