...
|---|---|---|
| **Test report** | The offline suite passes (no API key, no AWS) | `pytest demo platform_core/tests -q` |
| **SAST (bandit)** | No high-severity code security findings | `bandit -r platform_core governance -ll` |
| **Dependency audit (pip-audit)** | No known-vulnerable dependencies (**blocking** — deps hash-pinned in `platform_core/requirements-lock.txt`) | `pip-audit -r platform_core/requirements-lock.txt` |
