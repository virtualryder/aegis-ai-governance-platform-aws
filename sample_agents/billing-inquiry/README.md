...
Scaffolded Aegis agent for the **enterprise** pack, owned by **dept-finance**,
blast radius **low**.
...
4. Grounding KB `kb-enterprise-default` + threshold declared.
5. Token budget (hard cap) + inference profile `aip-enterprise-default`.
...
9. Pack `enterprise` declared; deploy fails if the pack is not active.
